...
export default function Sidebar({ conversations }: SidebarProps) {
  ...
  return (
    <div className="w-full md:w-80 bg-slate-800 border-r border-slate-700 flex flex-col h-screen">
      ...
      {/* User Profile Footer */}
      <div className="p-3 sm:p-4 border-t border-slate-700 bg-slate-800/50 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={() => setIsSettingsOpen(true)}
            className="p-2.5 sm:p-1.5 text-slate-400 hover:text-white hover:bg-slate-700 rounded-lg transition-colors min-h-[44px] min-w-[44px]"
            title="Edit Profile"
          >
            <svg
          ...
          <button
            onClick={() => signOut()}
            className="p-2.5 sm:p-1.5 text-slate-400 hover:text-red-400 hover:bg-slate-700 rounded-lg transition-colors min-h-[44px] min-w-[44px]"
            title="Logout"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
